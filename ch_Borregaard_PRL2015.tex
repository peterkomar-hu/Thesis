...
komar, acin}. Quantum gates can, in principle, also be directly implemented in
...
gates~\cite{duan1,duan2,Anders1prl, Anders2prl,ritter2014}, multi-qubit
...
Hamiltonian in a proper rotating frame is (see \reffig{fig:levels})
\begin{eqnarray}
\hat{H}&=&
\Delta_{E}\ket{E}
...
also verify our analytical results numerically~\cite{SM}. Assuming realisitc
parameters of $\kappa=100\gamma$ \cite{thompson,Tiecke}, we find that a driving
...
(see \cite{SM}). For both plots, we have assumed $\kappa=100\gamma$.}
\label{fig:prob}
\end{figure} 
...
in \reffig{fig:fidel_time}, assuming an initial state of
